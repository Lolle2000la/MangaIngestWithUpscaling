        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            if (migrationBuilder.ActiveProvider.Contains("Sqlite"))
            {
                migrationBuilder.Sql(
                    "CREATE INDEX IF NOT EXISTS IX_PersistedTasks_Type ON PersistedTasks (Data->>'$.$type')"
                );
                migrationBuilder.Sql(
                    "CREATE INDEX IF NOT EXISTS IX_PersistedTasks_ChapterId ON PersistedTasks (Data->>'$.ChapterId')"
                );
                migrationBuilder.Sql(
                    "CREATE INDEX IF NOT EXISTS IX_PersistedTasks_Type_ChapterId ON PersistedTasks (Data->>'$.$type', Data->>'$.ChapterId')"
                );
            }
        }
