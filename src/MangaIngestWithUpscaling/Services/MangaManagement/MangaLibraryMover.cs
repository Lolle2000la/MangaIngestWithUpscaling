...
) : IMangaLibraryMover
{
    ...
    )
    {
        ...
        var targetUpscaledLibraryPath =
            targetLibrary.UpscaledLibraryPath == null
                ? null
                : Path.Combine(
                    targetLibrary.UpscaledLibraryPath,
                    PathEscaper.EscapeFileName(manga.PrimaryTitle)
                );
        ...
        foreach (var chapter in manga.Chapters)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError(
                    ex,
                    "Failed to move chapter {fileName} from {sourcePath} to {targetPath}.",
                    chapter.FileName,
                    sourcePath,
                    targetPath
                );
            ...
            chapter.RelativePath = Path.GetRelativePath(
                targetLibrary.NotUpscaledLibraryPath,
                targetPath
            );
            ...
            if (!chapter.IsUpscaled)
                continue; // phew, we don't have to do anything for this chapter.
            ...
            string sourceUpscaledPath = Path.Combine(
                oldLibrary.UpscaledLibraryPath,
                chapter.RelativePath
            ); // this is again only a calculated property.
            ...
            {
                logger.LogWarning(
                    "Upscaled path for chapter {chapterId} is null. Skipping.",
                    chapter.Id
                );
                continue;
            }
            ...
            {
                logger.LogWarning(
                    "Target library {libraryId} does not have an upscaled library path. Skipping.",
                    targetLibrary.Id
                );
                continue;
            }
