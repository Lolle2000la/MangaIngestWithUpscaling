...
) : IIngestProcessor
{
    public async Task ProcessAsync(Library library, CancellationToken cancellationToken)
    {
        ...
        await foreach (FoundChapter originalChapter in foundChapters)
        {
            ...
            upscalerProfileDto = await upscalerJsonHandlingService.ReadUpscalerJsonAsync(
                fullPath,
                cancellationToken
            );
        ...
        foreach (var (series, processedItems) in chaptersBySeries)
        {
            ...
            Manga seriesEntity = await GetMangaSeriesEntity(
                library,
                series,
                originalSeriesTitle,
                cancellationToken
            ...
            bool shouldMergeChapterParts =
                seriesEntity.MergeChapterParts ?? library.MergeChapterParts;
            ...
            if (shouldMergeChapterParts && chapterPartMerger != null)
            {
                ...
                HashSet<string> existingChapterNumbers = seriesEntity
                ...
                HashSet<string> allChapterNumbers = existingChapterNumbers
                    .Union(newChapterNumbers)
                    .ToHashSet();
                ...
                if (mergeResult.MergeInformation.Any())
                {
                    ...
                    if (conflictingMergeInfos.Any())
                    {
                        logger.LogWarning(
                            ...
                            string.Join(
                                ", ",
                                conflictingMergeInfos.Select(m => m.BaseChapterNumber)
                            )
                        );
                        ...
                        {
                            string conflictingMergedFile = Path.Combine(
                                seriesDirectoryPath,
                            ...
                            if (File.Exists(conflictingMergedFile))
                            {
                                try
                                {
                                    ...
                                    logger.LogInformation(
                                        "Deleted conflicting merged file: {FileName}",
                                ...
                                catch (Exception ex)
                                {
                                    logger.LogError(
                                        ex,
                                        "Failed to delete conflicting merged file: {FileName}",
                                        conflictingMergedFile
                                    );
                                }
                            }
                ...
                logger.LogInformation(
                    ...
                    mergeResult.MergeInformation.Count,
                    mergedOriginalPartPaths.Count
                );
                ...
                foreach (FoundChapter processedChapter in mergeResult.ProcessedChapters)
                {
                    ...
                    bool isMergedChapter = mergeResult.MergeInformation.Any(mi =>
                        mi.MergedChapter.RelativePath == processedChapter.RelativePath
                    );
                    ...
                    if (!isMergedChapter)
                    {
                        ...
                        bool wasSupposedToBeMerged = mergedOriginalPartPaths.Contains(
                        ...
                        if (!wasSupposedToBeMerged)
                        {
                            ...
                            ProcessedChapterInfo? originalProcessedItem =
                                processedItems.FirstOrDefault(p =>
                                    p.Renamed.RelativePath == processedChapter.RelativePath
                                );
            ...
            foreach (ProcessedChapterInfo pci in finalProcessedItems)
            {
                ...
                if (pci.IsUpscaled) // Check original path for _upscaled marker
                {
                    ...
                    Chapter? foundMatch = await IngestUpscaledChapterIfMatchFound(
                        originalChapter,
                        renamedChapter,
                        seriesEntity,
                        library,
                ...
                try
                {
                    ...
                    {
                        logger.LogInformation(
                            "Updating metadata for {Path}. Old: {Old}, New: {New}",
                ...
                catch (Exception ex)
                {
                    logger.LogError(
                        ex,
                        "Error converting or updating metadata for {Path}",
                        originalChapter.RelativePath
                    );
                    continue;
                }
                ...
                {
                    logger.LogWarning(
                        "Chapter {File} exists at {Target}. Skipping.",
                        renamedChapter.FileName,
                        targetPath
                    );
                    continue;
                ...
                var relativeDbPath = Path.GetRelativePath(
                    library.NotUpscaledLibraryPath,
                    targetPath
                );
                if (
                    await dbContext.Chapters.AnyAsync(
                        c => c.RelativePath == relativeDbPath && c.Manga.Id == seriesEntity.Id,
                        cancellationToken
                    )
                )
                {
                    logger.LogWarning(
                        "Chapter {File} already in DB. Skipping.",
                        renamedChapter.FileName
                    );
                    continue;
                }
                ...
                if (library.FilteredImages.Any())
                {
                    try
                    {
                        var filterResult = await imageFilterService.ApplyFiltersToChapterAsync(
                            targetPath,
                        ...
                        {
                            logger.LogInformation(
                                "Filtered {Count} images from chapter {FileName} during ingest",
                                filterResult.FilteredCount,
                                chapterEntity.FileName
                            );
                        }
                    }
                    catch (Exception ex)
                    {
                        logger.LogError(
                            ex,
                            "Failed to apply image filters to {TargetPath}",
                            targetPath
                        );
                    }
                }
                ...
                if (renamedChapter.Metadata.ChapterTitle?.Contains("__MERGE_INFO__") == true)
                {
                    ...
                    if (parts.Length == 2)
                    {
                        ...
                        catch (Exception ex)
                        {
                            logger.LogError(
                                ex,
                                "Failed to save merge information for chapter {ChapterFile}",
                                renamedChapter.FileName
                            );
                        }
                    }
                ...
                if (
                    library.UpscaleOnIngest
                    && seriesEntity.ShouldUpscale != false
            ...
            if (mergeResult != null)
            {
                foreach (MergeInfo mergeInfo in mergeResult.MergeInformation)
                {
                    try
                    {
                        ...
                        string correctedRelativePath = Path.Combine(
                            seriesDirectoryName,
                            mergedChapter.FileName
                        );
                        string mergedTargetPath = Path.Combine(
                            library.NotUpscaledLibraryPath,
                        ...
                        logger.LogDebug(
                            "Merged chapter file verified at: {MergedTargetPath}",
                            mergedTargetPath
                        );
        ...
        logger.LogInformation(
            "Scanned {seriesCount} series in library {libraryName}. Cleaning.",
            chaptersBySeries.Count,
            library.Name
        );
    ...
    private async Task<Manga> GetMangaSeriesEntity(
        Library library,
        string newSeries,
    ...
    )
    {
        ...
        var seriesEntity = await dbContext
            ...
            .FirstOrDefaultAsync(
                s => s.PrimaryTitle == newSeries || s.OtherTitles.Any(an => an.Title == newSeries),
                cancellationToken: cancellationToken
            );
        ...
        if (seriesEntity == null)
        {
            ...
            if (!string.Equals(originalSeries, newSeries, StringComparison.Ordinal))
            {
                seriesEntity.OtherTitles.Add(
                    new MangaAlternativeTitle { Manga = seriesEntity, Title = originalSeries }
                );
            }
    ...
    private async Task<Chapter?> IngestUpscaledChapterIfMatchFound(
        FoundChapter originalUpscaled,
    ...
    )
    {
        ...
        if (!File.Exists(cbzPath))
        {
            logger.LogError(
                ...
                cbzPath,
                originalUpscaled.RelativePath,
                renamedUpscaled.RelativePath
            );
            return null;
        }
        ...
        if (
            nonUpscaledChapter == null
            && !string.IsNullOrEmpty(renamedUpscaled.Metadata.ChapterTitle)
        )
        {
            nonUpscaledChapter = seriesEntity.Chapters.FirstOrDefault(c =>
            {
                try
                {
                    ...
                    var existingNonUpscaledFilePath = Path.Combine(
                        library.NotUpscaledLibraryPath,
                        c.RelativePath
                    );
                    ...
                    var existingMetadata = metadataHandling.GetSeriesAndTitleFromComicInfo(
                        existingNonUpscaledFilePath
                    );
                ...
                catch (Exception ex)
                {
                    logger.LogError(
                        ...
                        Path.Combine(library.NotUpscaledLibraryPath, c.RelativePath),
                        c.Id
                    );
                    return false;
                }
        ...
        {
            if (
                upscalerProfileCache.TryGetValue(
                    upscalerProfileDto,
                    out UpscalerProfile? cachedProfile
                )
            )
            {
            ...
            else
            {
                ...
                profileToUse = await dbContext.UpscalerProfiles.FirstOrDefaultAsync(
                    p =>
        ...
        try
        {
            ...
            ExtractedMetadata finalDesiredMetadata = renamedUpscaled.Metadata with
            {
            ...
            if (library.FilteredImages.Any())
            {
                try
                {
                    ...
                    {
                        logger.LogInformation(
                            "Filtered {Count} images from upscaled chapter {FileName} during ingest",
                            filterResult.FilteredCount,
                            originalUpscaled.FileName
                        );
                    }
                }
                catch (Exception ex)
                {
                    logger.LogError(
                        ex,
                        "Failed to apply image filters to upscaled chapter {CbzPath}",
                        cbzPath
                    );
                }
            }
        ...
        var upscaleTargetFolder = Path.Combine(
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Error moving upscaled chapter {FoundFileName} to target path {TargetPath}.",
                originalUpscaled.FileName,
                upscaleTargetPath
            );
            return null;
