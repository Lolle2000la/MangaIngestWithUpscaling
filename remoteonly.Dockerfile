...
FROM mcr.microsoft.com/dotnet/aspnet10.0-noble AS base
WORKDIR /app
EXPOSE 8080
EXPOSE 8081
...
# configure models save directory (not used in remote-only mode but kept for consistency)
...
# Set remote-only execution mode - this disables local upscaling
...
LABEL org.opencontainers.image.description="MangaIngestWithUpscaling server component configured for remote-only execution without ML dependencies"
LABEL variant="remote-only"
