...
/// the local-wall-clock habit from creeping back in. It scans <c>*.cs</c> and <c>*.razor</c> under
/// <c>src/</c> and <c>tools/</c>.
/// </summary>
public class UtcDateTimeGuardTests
{
    ...
    public void SourceDoesNotUseLocalWallClockTime()
    {
        ...
        {
            // C# and Razor both run in-process, so a DateTime.Now in either is equally unsafe.
            foreach (string pattern in new[] { "*.cs", "*.razor" })
            {
                ...
                )
                {
                    ...
                    {
                        offenders.Add(
                            $"{Path.GetRelativePath(repositoryRoot, file)}: {match.Value}"
                        );
                    }
                }
            }
